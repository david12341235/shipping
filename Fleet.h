...
public:
    ...
    public:
        fleetInfo() : speed_(60.0), capacity_(1), cost_(1.0) {}
    };

    static Segment::Mode TypeInstance( Fwk::String );

    Mph speed( Segment::Mode m ) const {
        return fleet_.at(m).speed_;
    };
    void speedIs( Segment::Mode m, Mph _speed ) {
        fleet_[m].speed_ = _speed;
    }

    NumPackages capacity( Segment::Mode m ) const {
        return fleet_.at(m).capacity_;
    };
    void capacityIs( Segment::Mode m, NumPackages _capacity ) {
        fleet_[m].capacity_ = _capacity;
    }

    Dollar cost( Segment::Mode m ) const {
        return fleet_.at(m).cost_;
    };
    void costIs( Segment::Mode m, Dollar _cost ) {
        fleet_[m].cost_ = _cost;
    }
    ...
    public:
        ...
        Fleet::Ptr notifier() {
            return const_cast<Fleet *>(NotifieeConst::notifier().ptr());
        }
