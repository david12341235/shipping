...
void funnel( vector< Ptr<Instance> >& loc, vector< Ptr<Instance> >& seg, Ptr<Instance::Manager> manager )
{
...
void simulationStats( vector< Ptr<Instance> >& loc, vector< Ptr<Instance> >& seg )
{
	for( vector< Ptr<Instance> >::iterator i = loc.begin(); i != loc.end(); ++i )
	{
	}
}
